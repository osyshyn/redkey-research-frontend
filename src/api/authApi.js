...
export const loginAPI = async ({ email, fingerprint }) => {
...
export const checkMagicLinkAPI = async (token, { fingerprint }) => {
  console.log('fingerprint1', fingerprint);
  
  try {
    const response = await axiosInstance.post('auth/checkMagicLink', {
      token,
      fingerprint,
    });
